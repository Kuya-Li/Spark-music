<template>
	<div class="diy_details page_song_information" id="song_information_details">
		...
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_song_information'>
							<div_song_information :obj="obj"></div_song_information>
...
<script>
	import div_song_information from "@/components/diy/div_song_information.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "song_information_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/song_information/set?song_information_id=' + obj["song_information_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "song_information",
							source_field: "song_information_id",
							source_id: this.obj.song_information_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.song_information_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "song_information",
					source_field: "song_information_id",
					source_id: obj["song_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/song_information/set?song_information_id=' + _this.obj["song_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
