<template>
	<div class="diy_details page_hot_books" id="hot_books_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_hot_books'>
							<div_hot_books :obj="obj"></div_hot_books>
...
<script>
	import div_hot_books from "@/components/diy/div_hot_books.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/hot_books/set?hot_books_id=' + obj["hot_books_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.hot_books_id;
				}
			},
			/**
			 * 初始化收藏状态
