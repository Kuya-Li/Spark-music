<template>
	<div class="diy_details page_music_information" id="music_information_details">
		...
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_music_information'>
							<div_music_information :obj="obj"></div_music_information>
...
<script>
	import div_music_information from "@/components/diy/div_music_information.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "music_information_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/music_information/set?music_information_id=' + obj["music_information_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "music_information",
							source_field: "music_information_id",
							source_id: this.obj.music_information_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.music_information_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "music_information",
					source_field: "music_information_id",
					source_id: obj["music_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/music_information/set?music_information_id=' + _this.obj["music_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
