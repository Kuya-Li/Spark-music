<template>
	<div class="diy_details page_popular_songs" id="popular_songs_details">
		...
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_popular_songs'>
							<div_popular_songs :obj="obj"></div_popular_songs>
...
<script>
	import div_popular_songs from "@/components/diy/div_popular_songs.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/popular_songs/set?popular_songs_id=' + obj["popular_songs_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.popular_songs_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "popular_songs",
					source_field: "popular_songs_id",
					source_id: obj["popular_songs_id"],
					user_id
				};
