<template>
	<div class="diy_details page_music_data" id="music_data_details">
		...
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_music_data'>
							<div_music_data :obj="obj"></div_music_data>
...
<script>
	import div_music_data from "@/components/diy/div_music_data.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/music_data/set?music_data_id=' + obj["music_data_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "music_data",
							source_field: "music_data_id",
							source_id: this.obj.music_data_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.music_data_id;
				}
			},
			/**
			 * 初始化收藏状态
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/music_data/set?music_data_id=' + _this.obj["music_data_id"], {
								praise_len
							}, (res) => {
								if(res.result){
