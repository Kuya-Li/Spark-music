<template>
	<div class="diy_details_box diy_div_customized_goods">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
					...
					<div class="details_btn_wrap">
											<button class="btn btn-primary details_btn" v-if="$check_action('/i_want_to_customize/edit','add')" @click="to_form('/i_want_to_customize/edit')" > 定制 </button>
											<button class="btn btn-primary details_btn" v-if="$check_action('/customized_goods/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/customized_goods/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "customized_goods",
					source_field: "customized_goods_id",
					source_id: this.obj["customized_goods_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/customized_goods/set?customized_goods_id=' + _this.obj["customized_goods_id"], {
								praise_len
							}, (res) => {
								if(res.result){
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var customized_goods_id = this.obj.customized_goods_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "customized_goods",
          source_field: "customized_goods_id",
          source_id: customized_goods_id
        }, (res) => {
