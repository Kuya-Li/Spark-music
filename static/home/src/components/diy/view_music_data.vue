	...
	<el-main class="bg edit_wrap">
		...
		<el-row class="row_ce">
				...
				<el-form-item label="歌曲名称" prop="song_title">
							  ...
							  v-if="user_group === '管理员' || (form['music_data_id'] && $check_field('set','song_title')) || (!form['music_data_id'] && $check_field('add','song_title'))" :disabled="disabledObj['song_title_isDisabled']"></el-input>
				...
				<el-form-item label="歌手名称" prop="singer_name">
							  ...
							  v-if="user_group === '管理员' || (form['music_data_id'] && $check_field('set','singer_name')) || (!form['music_data_id'] && $check_field('add','singer_name'))" :disabled="disabledObj['singer_name_isDisabled']"></el-input>
				...
				<el-form-item label="专辑名称" prop="album_name">
							  ...
							  v-if="user_group === '管理员' || (form['music_data_id'] && $check_field('set','album_name')) || (!form['music_data_id'] && $check_field('add','album_name'))" :disabled="disabledObj['album_name_isDisabled']"></el-input>
				...
				<el-form-item label="发布时间" prop="release_time">
							  ...
							  v-if="user_group === '管理员' || (form['music_data_id'] && $check_field('set','release_time')) || (!form['music_data_id'] && $check_field('add','release_time'))" :disabled="disabledObj['release_time_isDisabled']"></el-input>
				...
				<el-form-item label="歌词内容" prop="lyrics_content">
						...
						v-if="user_group === '管理员' || (form['music_data_id'] && $check_field('set','lyrics_content')) || (!form['music_data_id'] && $check_field('add','lyrics_content'))" :disabled="disabledObj['lyrics_content_isDisabled']"></el-input>
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/music_data/view','set') || $check_action('/music_data/view','add') || $check_option('/music_data/table','examine')">
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "music_data_id",
				url_add: "~/api/music_data/add?",
				url_set: "~/api/music_data/set?",
				url_get_obj: "~/api/music_data/get_obj?",
				url_upload: "~/api/music_data/upload?",
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/music_data/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/music_data/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/music_data/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/music_data/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/music_data/view','get');
