<template>
	<div class="diy_details_box diy_div_popular_songs">
		<div class="warp diy_warp">
			...
			<div class="details_btn_wrap">
																											<button class="btn btn-primary details_btn" v-if="$check_action('/music_evaluation/edit','add')" @click="to_form('/music_evaluation/edit')" ><span> 评分</span> </button>
																											<button class="btn btn-primary details_btn" v-if="$check_action('/popular_songs/details','set')" @click="change_praise()" ><span>点赞</span></button>
																<button class="btn btn-primary details_btn" v-if="$check_action('/popular_songs/details','set')" @click="change_collect()" ><span>收藏</span></button>
											</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "popular_songs_id",
					source_id: this.obj["popular_songs_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/popular_songs/set?popular_songs_id=' + _this.obj["popular_songs_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.song_title
																													query.img = this.obj.singer_photos
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var popular_songs_id = this.obj.popular_songs_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "popular_songs",
          source_field: "popular_songs_id",
          source_id: popular_songs_id
        }, (res) => {
