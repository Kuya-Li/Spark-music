<template>
	<div class="diy_details_box diy_div_hot_books">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
					...
					<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/hot_books/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/hot_books/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "hot_books",
					source_field: "hot_books_id",
					source_id: this.obj["hot_books_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/hot_books/set?hot_books_id=' + _this.obj["hot_books_id"], {
								praise_len
							}, (res) => {
								if(res.result){
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var hot_books_id = this.obj.hot_books_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "hot_books",
          source_field: "hot_books_id",
          source_id: hot_books_id
        }, (res) => {
