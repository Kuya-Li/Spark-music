<template>
	<div class="diy_details_box diy_div_music_data">
		<div class="warp diy_warp">
			...
			<div class="details_btn_wrap">
																<button class="btn btn-primary details_btn" v-if="$check_action('/music_data/details','set')" @click="change_praise()" ><span>点赞</span></button>
																<button class="btn btn-primary details_btn" v-if="$check_action('/music_data/details','set')" @click="change_collect()" ><span>收藏</span></button>
											</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var music_data_id = this.obj.music_data_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "music_data",
          source_field: "music_data_id",
          source_id: music_data_id
        }, (res) => {
