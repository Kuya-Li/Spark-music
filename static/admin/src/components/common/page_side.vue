        ...
        @click="handleButtonClick(` ${i.mod_name}管理`)"><span>{{ i.page_title + "列表" }}</span></el-menu-item>
         <el-menu-item  v-if="i.has_add && (user_group == '管理员' || $check_action(i.path.replace(/(.*)table/,'$1view'), 'add'))"  :index="i.path.replace(/(.*)table/,'$1view')" :class="{ sactive: $route.path === i.path }"
          ...
          <template slot="title"><i class="el-icon-picture-outline"></i><span ref="customTemplate10">留言管理</span></template>
              <el-menu-item v-show="user_group == '管理员' || $check_action('/message/table')" index="/message/table" @click="handleButtonClick(10)">
                <span>留言板</span>
              </el-menu-item>
        </el-submenu>
          ...
          <template slot="title"><i class="el-icon-warning-outline"></i><span ref="customTemplate2">公告管理</span>
          </template>
          ...
          <el-menu-item v-show="user_group == '管理员' || $check_action('/forum/table')" index="/forum/table" @click="handleButtonClick(6)">
            <!-- <span>论坛</span> -->
            <span>{{$page_title("/forum/table") || "论坛"}}</span>
          </el-menu-item>
          <el-menu-item v-show="user_group == '管理员' || $check_action('/forum_type/table')" index="/forum_type/table" @click="handleButtonClick(6)">
            <!-- <span>论坛类型</span> -->
  ...
  export default {
    ...
    computed: {
      auth() {
        ...
        var lt = [];
        let site_list = []
                                                                                    site_list.push("music_evaluation")
                                                                                let list = ["exam", "article", "article_type", "comment", "forum", "forum_type", "notice", "message"];
